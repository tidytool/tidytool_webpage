...
/**
 * The quotes list on the admin order page: review line items, watch margin, move
 * status. Quote *generation* now lives in the top-of-page GenerateQuoteModal —
 * this section renders existing quotes only.
 *
 * The "Internal" box (cost estimate, gross margin) is for staff eyes only — if a
 * customer-facing quote view is ever built, it renders ONLY the lines, subtotal,
 * and total.
 */
    ...
    <section style={{ marginTop: "1.5rem" }}>
      ...
      {quotes.length === 0 ? (
        <p className="muted" style={{ fontSize: "0.9rem", marginTop: "0.4rem" }}>
          No quotes yet — use <strong>Generate quote</strong> at the top of the page.
        </p>
      ) : null}
          ...
          <div key={q.id} className="card" style={{ marginTop: "1rem", borderColor: stale ? "var(--c-warn, #c80)" : undefined }}>
            {stale ? (
              <p className="banner--warn" role="status" style={{ marginTop: 0 }}>
                ⚠ The order&rsquo;s boxes or drawer copies changed after this quote was priced. Regenerate for
                current pricing.
              </p>
            ) : null}
                        ...
                        ? l.qty != null && l.qty > 1 && l.unit_price_cents != null
                          ? (
                            <span className="muted">
                              {" "}
                              — {l.qty} × {formatCents(l.unit_price_cents)}
                            </span>
                          )
                          : null
                        : l.qty != null && l.unit
