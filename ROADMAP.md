...
- [x] **Supabase database hardening (security debt, deferred 2026-06-24).** _Done 2026-07-03: `0002_harden_anon_reads.sql` fully applied to prod. Verified: anon reads 0 rows from `drawer`/`employee`; `get_public_drawer(id)` still returns the public-safe row for a known id; deployed `docs/` pages only use RPCs. Remaining follow-ups (tracked, lower risk): revoke `is_admin()` EXECUTE from anon, drop the redundant duplicate `drawer` INSERT/UPDATE policies, fix `drawer-assets` bucket listing, enable leaked-password protection, Postgres patch upgrade._ Original scope:
